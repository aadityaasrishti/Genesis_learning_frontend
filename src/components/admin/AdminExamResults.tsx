...
const AdminExamResults: React.FC = () => {
  ...
  const calculateSummary = (data: StudentResult[]) => {
    if (!data.length) {
      setSummary(null);
      return;
    }
    ...
    data.forEach((student) => {
      student.results.forEach((result) => {
        ...
        highest = Math.max(highest, result.percentage);
        lowest = Math.min(lowest, result.percentage);
        totalResults++;
      });
    });

    setSummary({
      classAverage: totalResults ? totalPercentage / totalResults : 0,
      highestScore: highest,
      lowestScore: lowest === 100 && !totalResults ? 0 : lowest,
              ...
              <Card>
                <CardContent>
                  ...
                  <Typography variant="h4">
                    {summary.classAverage.toFixed(1)}%
                  </Typography>
                </CardContent>
              </Card>
            </Grid>
                ...
                {results.map((studentResult) => (
                  <React.Fragment key={studentResult.student.id}>
                    <TableRow>
                      ...
                      <TableCell>{studentResult.student.email}</TableCell>
